...
function makeWeeklyRecommendations(maxTemp, minTemp, maxUV, maxPrecipChance, maxWind, totalPrecipSum, minCloudCover) {
  let message = `<strong>Next Week</strong><br>`;
  message += `Based on the most extreme conditions forecasted this week, here’s what to prepare for:<br>`;
...
function makeWeatherChoices(maxTemperature, minTemperature, maxUVIndex, precipitationChance, windspeed, percipitationsum, cloudcover, date) {
  ...
  if (minTemperature < -10) {
    ...
    images.innerHTML += `<img src="bilder/jumper.jpg" height="150">`
  }

  if (maxUVIndex > 4) {
    message += `<br>It is imperative you wear sunscreen.`;
    images.innerHTML += `<img src="bilder/sunscreen.png" height="150">`
  ...
  if (precipitationChance > 25 && windspeed > 20) {
    ...
    images.innerHTML += `<img src="bilder/raincoat.jpg" height="150">`
    images.innerHTML += `<img src="bilder/umbrella.png" height="150">`
  }

  if (cloudcover < 30) {
    message += `<br>It could get quite bright. Bring sunglasses or a cap.`;
    images.innerHTML += `<img src="bilder/sunglasses.png" height="150">`
    images.innerHTML += `<img src="bilder/cap.jpg" height="150">`
  }
  
  message += `<br><br><strong>Daily Extremes:</strong><br>`
  message += `Max Temp: ${maxTemperature}°C, Min Temp: ${minTemperature}°C<br>`;
